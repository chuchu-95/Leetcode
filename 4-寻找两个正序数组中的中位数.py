...
class Solution:
    def findMedianSortedArrays(self, nums1: List[int], nums2: List[int]) -> float:
        ...
        while left <= right:
            ...
            nums1Left = float("-inf") if mid1 == 0 else nums1[(mid1-1)//2]
            nums1Right = float("inf") if mid1 == 2*n1 else nums1[mid1//2]
            nums2Left = float("-inf") if mid2 == 0 else nums2[(mid2-1)//2]
            nums2Right = float("inf") if mid2 == 2*n2 else nums2[mid2//2]
